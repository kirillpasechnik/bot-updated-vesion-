...
def do_something(update, context):
    ...
    if text == "present simple":
        update.message.reply_text(text="Это простое настоящее время, обозначающее действие в самом широком смысле этого слова. Одна из самых распространенных и простых форм в английском языке для описания действий.")
    elif text == "past simple":
        update.message.reply_text(text="Как образуется Past Simple? А очень просто. Не зря же его называют простым. Past Simple, образование: V2 (вторая форма глагола).")
    elif text == "past perfect":
        update.message.reply_text(text="Past Perfect — это прошедшее совершенное время в английском языке. Оно употребляется в случаях, когда мы рассказываем о каком-то действии, которое уже закончилось к определенному моменту времени в прошлом (или перед началом другого действия в прошлом).")
    elif text == "адрес":
